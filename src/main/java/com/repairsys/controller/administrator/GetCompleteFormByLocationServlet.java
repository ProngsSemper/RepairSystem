...
/**
 * @author Prongs
 * @date 2019/11/1 20:58
 */
@WebServlet("/admin/complete/location")
public class GetCompleteFormByLocationServlet extends BaseServlet {
    private final AdminServiceImpl adminService = ServiceFactory.getAdminService();
    private static final Logger logger = LoggerFactory.getLogger(GetCompleteFormByLocationServlet.class);

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        JSONObject requestBody = (JSONObject) request.getAttribute("requestBody");

        Result result = adminService.getCompleteFormByLocation(
                requestBody.getString("location"),
